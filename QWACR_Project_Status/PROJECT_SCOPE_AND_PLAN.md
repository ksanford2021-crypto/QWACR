...
**Status:** Active Development - Phases 1-4 COMPLETE; Phase 5 (Hardware Integration & Low-Level Drive Testing) IN PROGRESS
...
- ✅ PID velocity control tuned (current: Kp=18.0, Ki=0.2, Kd=0.05, Kff≈32.0)
- ✅ Sleep pin control (prevents startup motion)
- ✅ All motors tested: forward/reverse/turning functional
- ✅ Encoder feedback integrated (2× edge counting, 3200 CPR)
- ✅ Per-wheel current sensing and soft current limiting implemented (~2.6 A/channel)
...
**Deliverables:**
- ✅ Arduino firmware with request/response protocol for encoder + per-wheel current feedback
- ✅ ROS 2 hardware interface (mega_diff_drive_control) with odometry + TF via DiffDriveController
- ✅ Joint mapping and sign conventions fixed so `/joint_states` reflects the correct physical wheel and direction for each joint
- ✅ Twist→TwistStamped bridge (executable) and robust launch scripts (including serial warmup)
- ✅ Teleop keyboard driving hardware on the Pi and dev machine
...
- ✅ Hardware bring-up (GPS UART, LiDAR USB, TF setup)
  - Note: current platform uses an RPLIDAR S2 wired to the Raspberry Pi 5 UART on GPIO14/15 via `/dev/ttyAMA0`; these RX/TX pins are **reserved for LiDAR** and not available for additional UART peripherals.
...
### Phase 2c: Fire Sensor Payload Integration (Planned)
**Objective:** Add a dedicated fire-detection sensor payload for early smoke/heat/gas detection around the robot.

**Planned Components:**
- Sensirion SEN54 particulate/VOC/humidity/temperature module (SparkFun breakout, I²C + 5 V)
- MLX90640 32×24 thermal IR array (SparkFun, I²C + 3.3 V)
- SPEC Sensors DGS2 digital gas modules for H₂ and CO (UART)
- TCA9548A I²C multiplexer (Adafruit #2717) as an I²C hub for MLX90640 + SEN54

**Integration Strategy:**
- Use the Pi 5 primary I²C bus (GPIO2/3) to talk to the TCA9548A hub, with separate channels for the SEN54 and MLX90640.
- Configure **additional** UARTs on free GPIO pins via device-tree overlays for the two DGS2 modules, ensuring no conflict with the LiDAR UART on GPIO14/15.
- Expose all fire-sensor data as ROS 2 topics (particulates/VOC, humidity/temperature, thermal image, H₂ and CO gas concentrations).

See `QWACR_Project_Status/Fire_Sensor_Integration.md` for detailed wiring, pin assignments, and bring-up steps.
...
### Phase 5: Hardware Integration & Testing (Week 5-6) 🔄 IN PROGRESS
**Objective:** Integrate all sensors and validate complete navigation system, starting from robust low-level drive and feedback.

**Current Status (Feb 20, 2026):**
- Low-level 4-wheel drive, encoder feedback, and per-wheel current limiting are validated on hardware.
- ROS 2 hardware interface reliably publishes `/joint_states` with consistent positions/velocities and logs per-wheel currents from the Arduino.
- Robust launch scripts and Twist→TwistStamped bridge are working on the Raspberry Pi.

**Next Integration Steps:**
1. Aurora SLAM sensor connection and topic verification on the robot.
2. GPS driver launch and data quality checks on the robot.
3. EKF nodes launch and transform tree validation using real wheel odom + Aurora + GPS.
4. Motor control integration with Nav2 cmd_vel for closed-loop navigation.
5. Full system launch with all components for on-field trials.
...
- Short-range navigation goals (5–20 m).
- GPS waypoint accuracy testing.
- Long-range navigation (up to 800 m).

**Deliverables:**
- Hardware testing scripts for sensor validation and low-level drive characterization.
- System integration launch files (sensors, localization, full_system).
- Test procedures and checklists for on-robot Phase 5 sessions.
