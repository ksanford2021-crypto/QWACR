...
public:
  SerialProtocol() : command_ready(false), cmd_vel_left(0), cmd_vel_right(0) {}
  ...
  Command parseCommand(HardwareSerial& serial) {
    ...
    int packetSize = packetSerial.available();
    if (packetSize > 0) {
      byte buffer[64];
      packetSerial.read(buffer, packetSize);
      
      if (packetSize >= 9 && buffer[0] == CMD_SET_VELOCITY) {
        // Parse: [TYPE][vel_left_float][vel_right_float]
        memcpy(&cmd.vel_left, &buffer[1], 4);
        memcpy(&cmd.vel_right, &buffer[5], 4);
        cmd.valid = true;
      }
    }
