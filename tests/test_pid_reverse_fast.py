#!/usr/bin/env python3
"""
Test PID smoothness in reverse - ramp to -6 rad/s.
"""
import serial
import struct
import time
...
try:
    ...
    print("\n=== PID Reverse Speed Test ===")
    
    # Phase 1: Ramp down from 0 to -6 rad/s over 3 seconds
    print("\n1. Ramp REVERSE: 0 -> -6 rad/s (3 sec)")
    for i in range(30):
    ...
    # Phase 3: Step to -3 rad/s and hold
    print("\n3. Step UP: -6 -> -3 rad/s, hold (2 sec)")
    for i in range(20):
        send_velocity(ser, -3.0, 0.0)
        time.sleep(0.1)
    
    # Phase 4: Ramp to stop
    print("\n4. Ramp to STOP: -3 -> 0 rad/s (2 sec)")
    for i in range(20):
        vel = -3.0 + (i * 0.15)  # -3.0, -2.85, ... to ~0
        send_velocity(ser, vel, 0.0)
        if i % 5 == 0:
            print(f"  {vel:.1f} rad/s", end='\r')
        time.sleep(0.1)
